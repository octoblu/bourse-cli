...
class CommandItemUpdate
  constructor: ({@argv}) ->
    ...
    {@hostname, @username, @password, @item_id, @change_key, @email} = @parseOptions()
  ...
  parseOptions: =>
    ...
    {item_id, hostname, username, password, change_key, email} = options
    if _.some [item_id, hostname, username, password, change_key, email], _.isEmpty
      ...
      console.error colors.red 'Missing one of: -e, --email, BOURSE_EMAIL'   if _.isEmpty email
  ...
  run: =>
    ...
    bourse.forwardItem {itemId: @item_id, changeKey: @change_key, email: @email}, (error, item) =>
